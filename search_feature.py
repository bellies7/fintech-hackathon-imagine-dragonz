...
if __name__ == "__main__":
    ...
    # Test 1: Only one article matches one topic
    main_topics = {"inflation"}      # Only 'US inflation rises again' matches
    add_topics = None                # No add-topic bar

    results = search_articles(articles, main_topics, add_topics)
    print("Main Topics:", main_topics)
    print("Articles returned:")
    for r in results:
        print(f"- {r['title']} | Topics: {r['topics']}")

    print("\n---\n")

    # Test 2: Multiple articles match multiple topics (union)
    main_topics = {"inflation", "energy"}  # Matches US inflation + Oil prices
    add_topics = None
